...
Automatically detects conflicts between your Microsoft Edge browser policies and upcoming Edge releases, then posts an alert to a Microsoft Teams channel. Monitors the **Stable**, **Beta**, and **Extended Stable** channels — one alert per channel. No manual review required.
...
**How it works:**
An Azure Logic App runs weekly and:
1. Fetches the upcoming Edge release schedule from Microsoft's public API
2. For each channel (Stable, Beta, Extended Stable), fetches the latest release notes from Microsoft Docs
3. Reads your Edge policy list from a CSV file stored in Azure Blob Storage
4. Sends each channel's release notes + your policies to **Azure OpenAI (GPT-4o)** and asks it to identify conflicts
5. If conflicts are found, posts a separate Adaptive Card alert per channel to a Teams channel
...
2. **`Send-EdgePolicyAlert.ps1`** — fetches the Edge release schedule for all three channels, compares your exported policies against a known list of deprecated/changed policies per Edge version, and posts a separate Adaptive Card alert per channel to Teams if conflicts are found.
